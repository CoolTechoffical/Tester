...
app.post('/api/admin/login', async (req, res) => {
  ...
  const admin = await get(
    db,
    'SELECT username, password, display_name, role FROM admins WHERE username = ?',
    [username]
  );
...
app.post('/api/admin/holidays', requireAdmin, async (req, res) => {
  ...
  const result = await run(
    ...
    (date, title, category, district, subDistrict, scope, description, image, created_by) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
app.get('/api/summary', async (_req, res) => {
  ...
  const rainUpdates = holidays.filter(
    (item) => item.category === 'rain_holiday'
  );
